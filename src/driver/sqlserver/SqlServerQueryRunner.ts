...
export class SqlServerQueryRunner
    ...
    async renameTable(
        ...
        upQueries.push(
            new Query(
                `EXEC sp_rename "${this.escapePath(
                    oldTable,
                    true
                )}", "${newTableName}"`
            )
        ...
        upQueries.push(
            new Query(
                `ALTER TABLE ${this.escapePath(
                    table
                )} ADD ${this.buildCreateColumnSql(table, column, false, true)}`
            )
        );
        downQueries.push(
            new Query(
        ...
        if (
            ...
            if (newColumn.name !== oldColumn.name) {
                ...
                if (
                    ...
                    const oldDefaultName = this.connection.namingStrategy.defaultConstraintName(
                        table.name,
                        oldColumn.name
                    );
                    const newDefaultName = this.connection.namingStrategy.defaultConstraintName(
                        table.name,
                        newColumn.name
                    );
